...
st.set_page_config(page_title="21 Questions", page_icon="🃏", layout="centered")
...
st.markdown(
    ...
    <style>
    .stApp {
        background: radial-gradient(circle at 50% 10%, #18202d 0, #0d1118 48%, #080b10 100%);
    }
    [data-testid="stHeader"] { background: transparent; }
    .block-container { max-width: 760px; padding-top: 1.2rem; padding-bottom: 2rem; }
    h1 { color: #f7f7f8; text-align: center; letter-spacing: .04em; margin-bottom: .25rem; }
    .subtitle { color: #aab1bc; text-align: center; margin-bottom: 1rem; }
    </style>
    """,
...
question_bank_json = json.dumps(QUESTION_BANK, ensure_ascii=False)
...
.app { width: 100%; max-width: 620px; margin: 0 auto; padding: 4px 12px 18px; }
.tabs {
    display: grid;
...
.scene { display: flex; justify-content: center; perspective: 1400px; padding: 4px 0 10px; }
.card {
    position: relative;
...
.face {
    ...
    box-shadow: inset 0 0 0 2px rgba(73,31,28,.45), inset 0 0 26px rgba(46,17,16,.18);
}
.back {
    display: grid;
    place-items: center;
    color: #f4cf8e;
    background:
        radial-gradient(circle at 50% 50%, rgba(108,20,25,.22) 0 16%, transparent 16.5%),
        repeating-radial-gradient(circle at 52% 48%, rgba(255,255,255,.025) 0 1px, transparent 1px 4px),
...
.back::before {
    ...
    border-radius: 15px;
    box-shadow: inset 0 0 0 6px rgba(128,31,35,.78), inset 0 0 0 8px rgba(240,196,116,.42);
}
.sigil {
    position: relative;
    width: 62%;
    ...
    border-radius: 50%;
    box-shadow: 0 0 0 15px rgba(235,185,99,.08), 0 0 0 2px rgba(93,19,24,.9) inset;
}
...
.sigil::after { inset: 27%; border-radius: 50%; transform: none; }
.question-mark {
    position: relative;
    z-index: 2;
...
.front {
    ...
    color: #211d1a;
    background:
        radial-gradient(circle at 25% 12%, rgba(255,255,255,.96), transparent 38%),
        repeating-linear-gradient(15deg, rgba(120,74,41,.018) 0 1px, transparent 1px 5px),
...
.category { color: #c91f32; font-size: 18px; font-weight: 800; }
.divider {
    width: 58%;
    height: 1px;
    margin-top: 16px;
...
.number { color: rgba(68,45,35,.62); font-size: 13px; }
.hint { margin: 8px 0 12px; color: #aeb5c0; font-size: 13px; text-align: center; }
.next {
    width: 100%;
    min-height: 54px;
    border: 0;
...
@media (max-width: 520px) {
    ...
    .face { border-width: 5px; border-radius: 19px; }
    .front-content { padding: 31px 21px 36px; }
    .question { font-size: clamp(18px, 5.2vw, 22px); }
...
<div class="app">
    ...
    <div class="scene">
        <div id="card" class="card" role="button" tabindex="0" aria-label="翻牌或抽下一題">
            <div class="face back">
                <div class="sigil"><div class="question-mark">?</div></div>
            </div>
    ...
    <div class="hint">點選牌面可翻牌；顯示題目後再點一次可抽下一題</div>
...
function setCategory(nextCategory) {
    ...
    categoryLabel.textContent = category === "朋友" ? "♥ 朋友題" : "◆ 感情題";
    resetDeck();
}
...
html = html.replace("__BANK_JSON__", question_bank_json)
